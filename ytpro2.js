 ...
 * Original file: /npm/ytpro@3.3.22/script.js
 *
...
async function ytproDownVid() {
        ...
        var a = i.video.thumbnail?.thumbnails,
                ...
                l = i.video.title.replaceAll("|", "").replaceAll("\\", "").replaceAll("?", "").replaceAll("*", "").replaceAll("<", "").replaceAll("/", "").replaceAll(":", "").replaceAll('"', "").replaceAll(">", "");
        for (var s in t.innerHTML = "<style>#downytprodiv a{text-decoration:none;color:white;} #downytprodiv li{list-style:none; display:flex;align-items:center;justify-content:center;color:#fff;border-radius:25px;padding:10px;background:#000;margin:5px;}</style>", t.innerHTML += "Select Avilaible Formats<ul id='listurl'>", o) t.innerHTML += `<li data-ytprotit="${l}"  style="background:#001;"  onclick="YTDownVid(this,'.mp4')"  data-ytprourl="${o[s].url}">\n${downBtn}<span style="margin-left:10px;"  >${o[s].qualityLabel} ${formatFileSize(o[s].bitrate*(o[s].approxDurationMs/1e3)/8)} </span></li>`;
        for (s in r) r[s].mimeType.indexOf("audio") > -1 && (t.innerHTML += `<li data-ytprotit="${l}"  onclick="YTDownVid(this,'.mp3')"  data-ytprourl="${r[s].url}">\n${downBtn}<span style="margin-left:10px;"  >Audio${formatFileSize(r[s].contentLength)} \n</span></li>`);
        for (s in t.innerHTML += "<br>Thumbnails<br><br><style>.thu{height:80px;border-radius:10px;}.thu img{max-height:97%;max-width:70%;border-radius:10px;border:1px solid silver;}</style>", a) t.innerHTML += `<li data-ytprotit="${l+Date.now()}"  onclick="YTDownVid(this,'.png')" class="thu" data-ytprourl="${a[s].url}">\n<img src="${a[s].url}"><br>\n<span style="margin-left:30px;display:flex;align-items:center;justify-content:center;"  >${downBtn}<span style="margin-left:10px;"  >${a[s].height} &#x2715; ${a[s].width}\n</span></span></li>`
}
...
//# sourceMappingURL=/sm/5eccfd44e67f56704a2ba3ef1347eff460ec3d0bbb782793c88b4baf825c0968.map